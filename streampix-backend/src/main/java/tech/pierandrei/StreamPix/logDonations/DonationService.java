...
import tech.pierandrei.StreamPix.audio.AudioService;
import tech.pierandrei.StreamPix.gateway.MercadoPagoApiConfig;
import tech.pierandrei.StreamPix.gateway.PayloadMercadoPagoDTO;
import tech.pierandrei.StreamPix.gateway.ShortResponseApiDTO;
...
public class DonationService {
    ...
    public ShortResponseApiDTO donationService(DonationFullRequestDto dto, String nickname) throws Exception {
        ...
        try {
            var newLog = new LogDonationsEntity(
                    ...
                    Instant.now(),
                    dtoResponse.getId(),
                    LogStatusDonationEnum.PENDING_PAYMENT,
    ...
    public DonationInfoDTO getDonationInfoToPay(String id) {
        ...
        if (now.isAfter(expiresAt) && !donation.getStatusDonation().equals(LogStatusDonationEnum.SUCCESSFUL_PAYMENT)) {
